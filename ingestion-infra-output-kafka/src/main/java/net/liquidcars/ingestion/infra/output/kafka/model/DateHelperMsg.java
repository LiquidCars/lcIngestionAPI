...
public class DateHelperMsg {
    ...
    */
    ...
    private static long milisecondsFromSimOrigin(){
        if (DateHelperMsg.currentSimDateTime == null) {
            return 0;
        }
        else {
            return Instant.now().toEpochMilli() - DateHelperMsg.originSimDateTime.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
        }
    }
    ...
    public static LocalDateTime now(){
        return DateHelperMsg.currentSimDateTime!=null
                ? (DateHelperMsg.isSimRunningTime
                ? DateHelperMsg.currentSimDateTime.plusNanos(milisecondsFromSimOrigin() * 1_000_000)
                : DateHelperMsg.currentSimDateTime)
                : LocalDateTime.now();
    }

    public static String fromDateOnly(LocalDateTime dt){
        return  dt!=null ? dt.format(DateHelperMsg.DEFAULT_DATE_ONLY_FORMATTER) : null;
    }

    public static String nowDefault(){
        return DateHelperMsg.now().atZone(ZoneId.systemDefault()).format(DEFAULT_DATE_FORMATTER);
    }

    public static String nowUTCDefault(){
        return DateHelperMsg.now().atZone(ZoneOffset.UTC).format(DEFAULT_DATE_FORMATTER);
    }
    ...
    public static String dateDefault(LocalDateTime dt){
        if (dt!=null) {
            return dt.atZone(ZoneId.systemDefault()).format(DateHelperMsg.DEFAULT_DATE_FORMATTER);
        }
        else {
            return nowUTCDefault();
        }
