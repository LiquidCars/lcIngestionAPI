...
set COMPOSE_FILE_LIQUIDCARS=lcapi_docker_compose.yml
...
docker-compose -f %COMPOSE_FILE_LIQUIDCARS% --project-name liquidcars up -d

if %ERRORLEVEL% NEQ 0 (
    echo Docker Compose failed to start liquidcars.
    popd
    exit /b %ERRORLEVEL%
)
