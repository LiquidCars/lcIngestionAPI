...
public class DateHelperDto {
    ...
    */
    ...
    private static long milisecondsFromSimOrigin(){
        if (DateHelperDto.currentSimDateTime == null) {
            return 0;
        }
        else {
            return Instant.now().toEpochMilli() - DateHelperDto.originSimDateTime.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
        }
    }
    ...
    public static LocalDateTime now(){
        return DateHelperDto.currentSimDateTime!=null
                ? (DateHelperDto.isSimRunningTime
                ? DateHelperDto.currentSimDateTime.plusNanos(milisecondsFromSimOrigin() * 1_000_000)
                : DateHelperDto.currentSimDateTime)
                : LocalDateTime.now();
    }

    public static String fromDateOnly(LocalDateTime dt){
        return  dt!=null ? dt.format(DateHelperDto.DEFAULT_DATE_ONLY_FORMATTER) : null;
    }

    public static String nowDefault(){
        return DateHelperDto.now().atZone(ZoneId.systemDefault()).format(DEFAULT_DATE_FORMATTER);
    }

    public static String nowUTCDefault(){
        return DateHelperDto.now().atZone(ZoneOffset.UTC).format(DEFAULT_DATE_FORMATTER);
    }
    ...
    public static String dateDefault(LocalDateTime dt){
        if (dt!=null) {
            return dt.atZone(ZoneId.systemDefault()).format(DateHelperDto.DEFAULT_DATE_FORMATTER);
        }
        else {
            return nowUTCDefault();
        }
