...
public class IngestionReportDeleteInfraKafkaConsumer {
    ...
    public void consumeOffer(IngestionReportActionMsg message) {
        log.info("Received ingestion report job with id: {} for delete offers", message.getJobId());
        try {
            offerInfraKafkaConsumerService.processIngestionReportDeleteAction(UUID.fromString(message.getJobId()));
        } catch (Exception e) {
            ...
            throw LCIngestionException.builder()
                    ...
                    .message("Failed to process consumed job report with id: " + message.getJobId() + " for delete offers")
                    .cause(e)
                    .build();
        }
    }
