...
public class KeyValueJSONModel<K extends Serializable,V extends Serializable> extends DefaultKeyValue<K,V> implements Serializable{

    private static final String[] DEFAULT_KEYS = new String[] {"?", "UNKNOWN","OTHER","NONE"};
    public KeyValueJSONModel(){
        super();
    }
    public KeyValueJSONModel(final K key, final V value) {
        super(key, value);
    }

    public static KeyValueJSONModel getDefault(List<KeyValueJSONModel> KeyValueJSONModelList){
        if (KeyValueJSONModelList !=null && !KeyValueJSONModelList.isEmpty()) {
            return KeyValueJSONModelList.stream().filter(x-> {
    ...
    public static Map<String, String> toMap(List<KeyValueJSONModel> items) {
