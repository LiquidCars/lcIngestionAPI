...
@Component
public class DateHelperJSONModel {
    ...
    */
    ...
    private static long milisecondsFromSimOrigin(){
        if (DateHelperJSONModel.currentSimDateTime == null) {
            return 0;
        }
        else {
            return Instant.now().toEpochMilli() - DateHelperJSONModel.originSimDateTime.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
        }
    }
    ...
    public static LocalDateTime now(){
        return DateHelperJSONModel.currentSimDateTime!=null
                ? (DateHelperJSONModel.isSimRunningTime
                ? DateHelperJSONModel.currentSimDateTime.plusNanos(milisecondsFromSimOrigin() * 1_000_000)
                : DateHelperJSONModel.currentSimDateTime)
                : LocalDateTime.now();
    }

    public static String fromDateOnly(LocalDateTime dt){
        return  dt!=null ? dt.format(DateHelperJSONModel.DEFAULT_DATE_ONLY_FORMATTER) : null;
    }

    public static String nowDefault(){
        return DateHelperJSONModel.now().atZone(ZoneId.systemDefault()).format(DEFAULT_DATE_FORMATTER);
    }

    public static String nowUTCDefault(){
        return DateHelperJSONModel.now().atZone(ZoneOffset.UTC).format(DEFAULT_DATE_FORMATTER);
    }
    ...
    public static String dateDefault(LocalDateTime dt){
        if (dt!=null) {
            return dt.atZone(ZoneId.systemDefault()).format(DateHelperJSONModel.DEFAULT_DATE_FORMATTER);
        }
        else {
            return nowUTCDefault();
        }
