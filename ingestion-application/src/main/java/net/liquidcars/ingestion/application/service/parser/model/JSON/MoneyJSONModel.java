...
@Data
@Schema(title="The Money JSON object", description="A representation of money")
@JsonPropertyOrder({"amount", "currency"})
public class MoneyJSONModel implements Serializable, Comparable<MoneyJSONModel> {
    ...
    @Builder
    public MoneyJSONModel(@JsonProperty("amount") BigDecimal amount,
    ...
    public static MoneyJSONModel toMoney(double amount, String currencyId) {
        BigDecimal value = BigDecimal.valueOf(amount)
                .setScale(2, RoundingMode.HALF_EVEN);
        return new MoneyJSONModel(value, currencyId);
    }

    public static MoneyJSONModel toMoney(BigDecimal amount, String currencyId) {
        if (amount == null) return new MoneyJSONModel(null, currencyId);
        BigDecimal value = amount.setScale(2, RoundingMode.HALF_EVEN);
        return new MoneyJSONModel(value, currencyId);
    }

    public static MoneyJSONModel toMoney(long amount, String currencyId) {
        BigDecimal value = BigDecimal.valueOf(amount)
                .setScale(2, RoundingMode.HALF_EVEN);
        return new MoneyJSONModel(value, currencyId);
    }

    public static MoneyJSONModel fromMoney(MoneyJSONModel source)
    {
        return new MoneyJSONModel(source.getAmount(), source.getCurrency());
    }


    public MoneyJSONModel plus(MoneyJSONModel amount) {
        if (!this.getCurrency().equals(amount.getCurrency())) return null;
        BigDecimal result = this.getAmount().add(amount.getAmount())
                .setScale(2, RoundingMode.HALF_EVEN);
        return new MoneyJSONModel(result, this.getCurrency());
    }

    public MoneyJSONModel minus(MoneyJSONModel amount) {
        if (!this.getCurrency().equals(amount.getCurrency())) return null;
    ...
    public MoneyJSONModel multiply(double multiplier) {
    ...
    public MoneyJSONModel multiply(long amount) {
        BigDecimal result = this.getAmount()
    ...
    public boolean isComparableWith(MoneyJSONModel amount){
    ...
    public int compareToExtended(MoneyJSONModel amount){
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof MoneyJSONModel)) {
            return false;
        }

        // typecast o to Money so that we can compare data members
        MoneyJSONModel c = (MoneyJSONModel) o;
    ...
    @Override
    public int compareTo(MoneyJSONModel amount) {
        if (!isComparableWith(amount)) {
